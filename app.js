...
// Robust available-balance: prefer local compute, but fall back to displayed text if larger
function availableBalance(){
  try{
    let local = Number(computeBalance()||0);
    if(!Number.isFinite(local)) local = 0;
    let shown = NaN;
    try{ const t = (document.getElementById('balance')||{}).textContent || ''; shown = parseAmount(String(t)); }catch{}
    if(Number.isFinite(shown)) return Math.max(local, shown);
    return local;
  }catch{ return 0 }
}
function save(){
...
function computeBalance(){
  ...
  // goals 縺ｮ繝ｭ繝ｼ繧ｫ繝ｫ繝溘Λ繝ｼ縺ｨ繧､繝吶Φ繝磯夂衍・郁ｵｷ蜍墓凾縺ｮ繧ｭ繝｣繝・す繝･荳頑嶌縺阪ｒ髦ｲ縺舌◆繧・ｼ・  function mirrorGoalsCache(){
    try{
  ...
  function contributeToGoal(goal){
    ...
    if(max <= 0){
      ...
      if(approx <= 0 && (state.transactions||[]).length===0){
        return toast('縺ｾ縺壹・縺翫％縺･縺九＞繧偵◆繧√ｈ縺・ｼ・);
      }
    }
    ...
    save(); mirrorGoalsCache();
    // 蜿肴丐: 繧ゅ￥縺ｲ繧・≧ 縺ｨ 縺｡繧・″繧・縺ｮ荳｡譁ｹ繧貞・謠冗判
    renderGoals();
    renderSavings();
...
function deleteGoal(goal){
    ...
    try{
      ...
      if(__refund > 0){ addTx('income', __refund, `繧ゅ←縺・ ${goal.name}`, true); }
    }catch{}
    save(); mirrorGoalsCache();
    renderGoals();
  }
...
try{
  window.kidsAllowanceApplyGoals = function(goals){
    try{
      ...
      // If incoming is empty and譌｢蟄倥↓繧ｴ繝ｼ繝ｫ縺後≠繧後・菫晄戟・育ｩｺ縺ｧ荳頑嶌縺阪＠縺ｪ縺・ｼ・      if(arr.length===0 && Array.isArray(state.goals) && state.goals.length>0){
